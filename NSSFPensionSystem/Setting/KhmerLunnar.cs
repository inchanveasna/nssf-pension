...
{
    public class KhmerLunnar
    {
		public class KhmerLunar
		{
			public static string getCalendarLeap(int BEYear)
			{
				string leap = KhmerLunar.getBoditheyLeap(BEYear);
				string leapP = KhmerLunar.getBoditheyLeap(BEYear - 1);
				if (leap.Equals("MD"))
				{
					return "M";
				}
			...
			public static string getBoditheyLeap(int BEYear)
			{
				...
				int arg_2A_0 = 800 - Aharkun_mod;
				bool isKhmerSolarLeap = false;
				if (arg_2A_0 <= 207)
				{
					isKhmerSolarLeap = true;
				}
				int aharkun = (int)(Math.Floor((double)(BEYear * 292207 + 499)) / 800.0) + 4;
				int avoman = (aharkun * 11 + 25) % 692;
				int aharkun_n = (int)(Math.Floor((double)((BEYear + 1) * 292207 + 499)) / 800.0) + 4;
				int avoman_n = (aharkun_n * 11 + 25) % 692;
				double arg_B3_0 = Math.Floor((double)((BEYear + 1) * 292207 + 499)) / 800.0;
				...
				int bodithey = ((int)Math.Floor((double)((aharkun * 11 + 25) / 692)) + aharkun + 29) % 30;
				int bodithey_n = ((int)Math.Floor((double)((aharkun_n * 11 + 25) / 692)) + aharkun_n + 29) % 30;
			...
			{
				Hashtable arg_97_0 = KhmerLunar.getHashMonth();
				Hashtable hsAnimalYear = KhmerLunar.getHashAnimalYear();
				Hashtable arg_83_0 = KhmerLunar.getHashSak();
				string enText = KhmerLunar.getKhmerLunarCode(srcDate);
				...
				sak = arg_83_0[sak].ToString();
				year = KhmerLunar.convertToKhmerNum(year);
				month = arg_97_0[month].ToString();
				animalYear = hsAnimalYear[animalYear].ToString();
				kr = kr.Replace("K", "កើត").Replace("R", "រោច");
				d = KhmerLunar.convertToKhmerNum(int.Parse(d).ToString());
			...
			public static string getKhmerLunarCode(DateTime srcDate)
			{
				...
				{
					string leap = KhmerLunar.getCalendarLeap(tmpDate.AddDays((double)i).Year + 544);
					cNum++;
					if (cNum == 207 && !(leap == "D"))
					{
						cNum = 208;
				...
				while (tmpNewYearDay <= 17)
				{
					...
					if (tmp.DayOfWeek == (DayOfWeek)dayOfNewYear)
					{
						break;
					}
					tmpNewYearDay++;
				...
				else
				{
					...
					sak = srcDate.Year % 10 + 1;
				}
				string result = hsDay[cNum].ToString();
				if (result == "07R14" && KhmerLunar.getCalendarLeap(srcDate.Year + 544) != "D")
				{
					result += "S";
				}
				return sak.ToString("00") + animalYear.ToString("00") + yearPath.ToString("0000") + result;
			}
		}
	}
}
